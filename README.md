...
This claim names no model, no dataset, and no number:
...
This claim must be true or false independent of Qwen3-0.6B, independent of MMLU-Redux, independent of the anatomy subset. If it only holds under the paper's exact experimental setup, it is not a claim about a mechanism. It is an observation about one setup.
...
Outcome B closes Misi 0 as validly as outcome A does. There is no obligation to scale up the model to "prove it again" once outcome B has been reached with measurement that can be defended. Scaling up, as a positive control on a larger model, is a separate follow-on question, not part of Misi 0's exit condition.

## Why outcome C requires work absent from the old checklist

The paper's own gap between Direct and Oracle is roughly four percentage points. A single point estimate from one run cannot distinguish outcomes A, B, and C. A four-point difference may be a real signal or a sampling coincidence, depending on the variance underneath it. Because Direct and Oracle are run on the exact same questions, this comparison is paired. One of the following two measures of uncertainty is required before A or B can be claimed:
...
The MMLU-Redux gold answer measures whether the model understood the question. That is not the same measurement as whether the extractor read the model's raw output correctly. The extractor is validated against a small, hand-labeled probe set, read by a human against raw text, before it is trusted to run on the full N. These two layers are validated separately, in this order, before any accuracy number is trusted.

## Boundary: where the paper starts binding again

This document applies only to Misi 0. Starting at Misi 1, the goal changes. It is no longer testing whether a general claim about cache enrichment holds. It is building an artifact that is architecturally comparable to C2C: two distinct models, terminal layer alignment, an MLP projector shaped as the paper's Appendix A.3.2 describes. There, fidelity to the paper's design choices is not about chasing a number. It is about building something structurally comparable to what they claim succeeded. Different goal, different rules.
...
The steps are ordered. Each one gates the next; skipping ahead is how the extractor problems in the parent project happened in the first place.
...
5. **Run the full experiment.**
   ```
   python run_experiment.py --n 100 --out results/run_<date>.json
   ```
...
6. **Analyze the result.**
   ...
   ```
   This reports the paired accuracy difference along with its bootstrap confidence interval or McNemar statistic, and states which of the three outcomes (A, B, or C, defined above) the run supports.
